...
const Booking = (props) => {
    ...
    const [startDate, setStartDate] = useState(getDateTowShow({ startDate: new Date() }))
    ...
    useEffect(() => { if (!appointment.person) router.push("/step1") }, [])
...
const ListFreeSlots = ({ slots, type = 20 }) => {
    ...
    for (const i = 0; i < slots.length; i++) {
        ...
        let gbs = slots[i].employee_settings.filter(e => e.eType === type && !e.blocked && !e.closed && !e.stats)
...
const ShowDates = ({ dates, allSlots }) => {
    ...
    return <div style={{ display: 'flex', justifyContent: "space-between", "marginTop": "30px" }}>
        {cols}
    </div>
}
